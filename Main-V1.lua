...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>vanhdzut hub chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "vanhdzut hub", HidePremium = false, IntroText = "vanhdzut hub⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("vanhdzut hub SCRIPT")
W:AddLabel("Discordvanhdzut hub")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "vanhdzut hub(Noitification)",
